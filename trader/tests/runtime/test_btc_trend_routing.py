...
class TestBtcTrendRouting:
    def test_daily_fallback_uses_latest_daily_candle(self, mock_bot):
        ...
        assert resolved['reason'] == 'daily_ema'
    ...
    def test_regime_probe_is_independent_from_daily_trend_resolution(self, mock_bot, caplog):
    ...
    def test_ambiguous_regime_does_not_override_daily_trend(self, mock_bot):
        ...
        with patch.object(mock_bot.data_provider, 'fetch_ohlcv', return_value=_make_btc_df(True)):
            resolved = mock_bot._resolve_btc_trend_context()

        assert resolved['trend'] == 'LONG'
        assert resolved['source'] == '1d_fallback'
        assert resolved['reason'] == 'daily_ema'

    def test_daily_path_missing_stays_unknown(self, mock_bot, caplog):
        with patch.object(mock_bot.data_provider, 'fetch_ohlcv', return_value=pd.DataFrame()):
            caplog.set_level('INFO')
            resolved = mock_bot._resolve_btc_trend_context(log_event=True)
    ...
    def test_ranging_regime_does_not_block_daily_trend_resolution(self, mock_bot):
        mock_bot._btc_regime_context = mock_bot._make_btc_context(
            source='regime',
            regime='RANGING',
            detected='RANGING',
        ...
        with patch.object(mock_bot.data_provider, 'fetch_ohlcv', return_value=_make_btc_df(False)):
            resolved = mock_bot._resolve_btc_trend_context()
