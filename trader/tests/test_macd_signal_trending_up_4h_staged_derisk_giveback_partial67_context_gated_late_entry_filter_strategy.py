...
from trader.strategies.plugins.macd_signal_trending_up_4h_staged_derisk_giveback_partial67_context_gated_late_entry_filter import (
    MacdSignalTrendingUp4hStagedDeriskGivebackPartial67ContextGatedLateEntryFilterStrategy,
)
...
def test_registry_loads_macd_signal_trending_up_4h_partial67_context_gated_late_entry_filter_plugin():
...
def test_context_gated_late_entry_filter_allows_overextended_entry_when_context_is_inactive():
    plugin = (
        MacdSignalTrendingUp4hStagedDeriskGivebackPartial67ContextGatedLateEntryFilterStrategy()
    )
    ...
    assert intent.entry_type == "macd_signal_cross_up_context_gated_late_entry_filtered"
...
def test_context_gated_late_entry_filter_blocks_overextended_entry_when_trend_decay_is_active():
    plugin = (
        MacdSignalTrendingUp4hStagedDeriskGivebackPartial67ContextGatedLateEntryFilterStrategy()
    )
...
def test_context_gated_late_entry_filter_blocks_overextended_entry_when_chop_trend_is_active():
    plugin = (
        MacdSignalTrendingUp4hStagedDeriskGivebackPartial67ContextGatedLateEntryFilterStrategy()
    )
    trend_frame = _trend_frame_with_tail_spreads([0.012, 0.013, 0.014, 0.015])
...
def test_context_gated_late_entry_filter_keeps_entry_when_context_is_active_but_stretch_is_small():
    plugin = (
        MacdSignalTrendingUp4hStagedDeriskGivebackPartial67ContextGatedLateEntryFilterStrategy()
    )
    trend_frame = _trend_frame_with_tail_spreads([0.015, 0.014, 0.013, 0.012])
...
def test_context_gated_late_entry_filter_preserves_partial67_management():
    plugin = (
        MacdSignalTrendingUp4hStagedDeriskGivebackPartial67ContextGatedLateEntryFilterStrategy()
    )
    trend_frame = _trend_frame_with_tail_spreads([0.015, 0.014, 0.013, 0.012])
