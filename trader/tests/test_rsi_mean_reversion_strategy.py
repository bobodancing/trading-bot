...
from trader.strategies.plugins.rsi_mean_reversion import RsiMeanReversionStrategy
...
def test_registry_loads_rsi_mean_reversion_plugin():
    registry = StrategyRegistry.from_config(
        get_strategy_catalog(["rsi_mean_reversion_15m"]),
        ["rsi_mean_reversion_15m"],
    )

    plugin = registry.require("rsi_mean_reversion_15m")
    assert isinstance(plugin, RsiMeanReversionStrategy)
...
def test_rsi_mr_emits_long_on_oversold_with_bb_and_low_adx():
    plugin = RsiMeanReversionStrategy(params={"symbol": "BTC/USDT", "stop_atr_mult": 1.5})
    frame = _frame(
    ...
    intent = intents[0]
    assert intent.strategy_id == "rsi_mean_reversion_15m"
...
def test_rsi_mr_requires_rsi_below_threshold():
    plugin = RsiMeanReversionStrategy(params={"symbol": "BTC/USDT"})
    frame = _frame(
        closes=[101.0, 100.0, 99.0, 98.0, 97.0, 100.0],
...
def test_rsi_mr_requires_bb_lower_touch():
    plugin = RsiMeanReversionStrategy(params={"symbol": "BTC/USDT"})
    frame = _frame(
...
def test_rsi_mr_blocks_in_trending_market():
    plugin = RsiMeanReversionStrategy(params={"symbol": "BTC/USDT"})
    frame = _frame(
        closes=[101.0, 100.0, 99.0, 98.0, 97.0, 100.0],
        rsi=[40.0, 38.0, 32.0, 28.0, 24.0, 20.0],
...
def test_rsi_mr_emit_once_blocks_same_candle_duplicate():
    plugin = RsiMeanReversionStrategy(
...
def test_rsi_mr_cooldown_blocks_within_n_bars():
    plugin = RsiMeanReversionStrategy(params={"symbol": "BTC/USDT", "cooldown_bars": 5, "emit_once": False})
    base = _frame(
        closes=[101.0, 100.0, 99.0, 98.0, 97.0, 100.0],
        rsi=[40.0, 38.0, 32.0, 28.0, 24.0, 20.0],
        bb_lower=[102.0, 101.0, 100.0, 99.0, 98.0, 100.0],
...
def test_rsi_mr_skips_unsupported_symbol():
    plugin = RsiMeanReversionStrategy()
    frame = _frame()
...
def test_rsi_mr_handles_nan_indicators():
    plugin = RsiMeanReversionStrategy(params={"symbol": "BTC/USDT"})
    frame = _frame()
...
def test_rsi_mr_exit_on_rsi_recovery():
    plugin = RsiMeanReversionStrategy(params={"symbol": "BTC/USDT"})
    frame = _frame(
        closes=[101.0, 100.0, 99.0, 98.0, 97.0, 101.0],
...
def test_rsi_mr_exit_on_bb_mid_cross():
    plugin = RsiMeanReversionStrategy(params={"symbol": "BTC/USDT"})
    frame = _frame(
...
def test_rsi_mr_exit_on_adx_trend_onset():
    plugin = RsiMeanReversionStrategy(params={"symbol": "BTC/USDT"})
    frame = _frame(
...
def test_rsi_mr_hold_when_still_oversold():
    plugin = RsiMeanReversionStrategy(params={"symbol": "BTC/USDT"})
    frame = _frame(
        closes=[101.0, 100.0, 99.0, 98.0, 97.0, 99.0],
...
def test_rsi_mr_stop_hint_math():
    plugin = RsiMeanReversionStrategy(params={"symbol": "BTC/USDT", "stop_atr_mult": 1.5})
    frame = _frame(
        closes=[101.0, 100.0, 99.0, 98.0, 97.0, 100.0],
        rsi=[40.0, 38.0, 32.0, 28.0, 24.0, 20.0],
