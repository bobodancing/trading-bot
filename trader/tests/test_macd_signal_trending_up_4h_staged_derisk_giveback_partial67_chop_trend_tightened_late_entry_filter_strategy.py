...
from trader.strategies.plugins.macd_signal_trending_up_4h_staged_derisk_giveback_partial67_chop_trend_tightened_late_entry_filter import (
    MacdSignalTrendingUp4hStagedDeriskGivebackPartial67ChopTrendTightenedLateEntryFilterStrategy,
)
...
def test_registry_loads_macd_signal_trending_up_4h_partial67_chop_trend_tightened_late_entry_filter_plugin():
    ...
    assert plugin.params["chop_trend_extension_atr_trigger"] == pytest.approx(1.5)
    assert {"adx", "bbw"}.issubset(plugin.required_indicators)


def test_chop_trend_tightened_late_entry_filter_allows_overextended_entry_when_chop_trend_is_inactive():
    plugin = (
        MacdSignalTrendingUp4hStagedDeriskGivebackPartial67ChopTrendTightenedLateEntryFilterStrategy()
    )
    ...
    assert (
        intent.entry_type
        == "macd_signal_cross_up_chop_trend_tightened_late_entry_filtered"
    )
    assert intent.metadata["weak_tape_gate_mode"] == "chop_trend_tightened"
...
def test_chop_trend_tightened_late_entry_filter_keeps_moderate_overextension_when_chop_trend_is_active():
    plugin = (
        MacdSignalTrendingUp4hStagedDeriskGivebackPartial67ChopTrendTightenedLateEntryFilterStrategy()
    )
    ...
    assert intent.metadata["weak_tape_context_active"] is False
    assert intent.metadata["weak_tape_chop_trend_veto_active"] is False
    assert intent.metadata["entry_extension_atr"] == pytest.approx(1600.0 / 1200.0)
    assert (
        intent.metadata["entry_extension_atr"]
        < intent.metadata["weak_tape_chop_trend_extension_atr_trigger"]
    )


def test_chop_trend_tightened_late_entry_filter_blocks_extreme_overextended_entry_when_chop_trend_is_active():
    plugin = (
        MacdSignalTrendingUp4hStagedDeriskGivebackPartial67ChopTrendTightenedLateEntryFilterStrategy()
    )
    trend_frame = _trend_frame_with_tail_spreads([0.012, 0.013, 0.014, 0.015])
...
def test_chop_trend_tightened_late_entry_filter_preserves_partial67_management():
    plugin = (
        MacdSignalTrendingUp4hStagedDeriskGivebackPartial67ChopTrendTightenedLateEntryFilterStrategy()
    )
    trend_frame = _trend_frame_with_tail_spreads([0.012, 0.013, 0.014, 0.015])
