...
from trader.strategies.plugins.macd_signal_trending_up_4h_staged_derisk_giveback_partial67_trend_decay_only_late_entry_filter import (
    MacdSignalTrendingUp4hStagedDeriskGivebackPartial67TrendDecayOnlyLateEntryFilterStrategy,
)
...
def test_registry_loads_macd_signal_trending_up_4h_partial67_trend_decay_only_late_entry_filter_plugin():
    ...
    assert plugin.params["weak_tape_trend_spread_delta_max"] == pytest.approx(0.0)


def test_trend_decay_only_late_entry_filter_allows_overextended_entry_when_trend_decay_is_inactive():
    plugin = (
        MacdSignalTrendingUp4hStagedDeriskGivebackPartial67TrendDecayOnlyLateEntryFilterStrategy()
    )
    ...
    assert intent.entry_type == "macd_signal_cross_up_trend_decay_only_late_entry_filtered"
    assert intent.metadata["weak_tape_gate_mode"] == "trend_decay_only"
    assert intent.metadata["weak_tape_context_active"] is False
    assert intent.metadata["weak_tape_context_reason"] == "none"
    assert intent.metadata["weak_tape_chop_trend_active"] is False
    assert intent.metadata["entry_extension_atr"] > intent.metadata["entry_ema_extension_atr_max"]


def test_trend_decay_only_late_entry_filter_blocks_overextended_entry_when_trend_decay_is_active():
    plugin = (
        MacdSignalTrendingUp4hStagedDeriskGivebackPartial67TrendDecayOnlyLateEntryFilterStrategy()
    )
...
def test_trend_decay_only_late_entry_filter_ignores_chop_trend_activation():
    plugin = (
        MacdSignalTrendingUp4hStagedDeriskGivebackPartial67TrendDecayOnlyLateEntryFilterStrategy()
    )
    trend_frame = _trend_frame_with_tail_spreads([0.012, 0.013, 0.014, 0.015])
...
def test_trend_decay_only_late_entry_filter_keeps_entry_when_trend_decay_is_active_but_stretch_is_small():
    plugin = (
        MacdSignalTrendingUp4hStagedDeriskGivebackPartial67TrendDecayOnlyLateEntryFilterStrategy()
    )
    trend_frame = _trend_frame_with_tail_spreads([0.015, 0.014, 0.013, 0.012])
    ...
    assert intent.metadata["weak_tape_context_reason"] == "trend_decay"
    assert intent.metadata["weak_tape_trend_decay_active"] is True
    assert intent.metadata["entry_extension_atr"] == pytest.approx(500.0 / 1200.0)


def test_trend_decay_only_late_entry_filter_preserves_partial67_management():
    plugin = (
        MacdSignalTrendingUp4hStagedDeriskGivebackPartial67TrendDecayOnlyLateEntryFilterStrategy()
    )
    trend_frame = _trend_frame_with_tail_spreads([0.015, 0.014, 0.013, 0.012])
