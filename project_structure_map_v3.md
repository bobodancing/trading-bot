...
**Dependencies:** `json, logging, os, socket, subprocess, time, urllib.parse, urllib.request`
...
### Class: `MarketScanner` — 市場掃描器主類
  ...
  - Method: `scan(self)` -> Tuple[List[ScanResult], MarketSummary] [Calls: layer2_momentum_filter, layer1_liquidity_filter, _output_results, layer3_pattern_matching, layer4_correlation_filter, _generate_market_summary] — 執行完整掃描
  - Method: `_generate_market_summary(self)` -> MarketSummary — 生成市場摘要
  - Method: `_output_results(self)` [Calls: _print_summary, _output_sqlite, _send_telegram, _output_json] — 輸出掃描結果
...
## 📄 File: `trader/bot.py`
**Dependencies:** `sys, os, time, json, signal, logging, logging.handlers, pathlib.(Path), datetime.(datetime, timezone), typing.(Dict, List, Optional), ccxt, pandas, trader.infrastructure.api_client.(BinanceFuturesClient), trader.infrastructure.notifier.(TelegramNotifier), trader.infrastructure.telegram_handler.(TelegramCommandHandler), trader.infrastructure.data_provider.(MarketDataProvider), trader.infrastructure.performance_db.(PerformanceDB), trader.indicators.technical.(TechnicalAnalysis, DynamicThresholdManager, MTFConfirmation, MarketFilter), trader.risk.manager.(PrecisionHandler, RiskManager, SignalTierSystem), trader.execution.order_engine.(OrderExecutionEngine), trader.config.(ConfigV6), trader.positions.(PositionManager), trader.persistence.(PositionPersistence), trader.signals.(detect_2b_with_pivots, detect_ema_pullback, detect_volume_breakout), trader.strategies.base.(Action), argparse`
...
### Class: `TradingBotV6` — V6.0 終極滾倉版交易機器人
  ...
  - Method: `scan_for_signals(self)` [Calls: _check_btc_trend, _execute_trade, load_scanner_results, fetch_ohlcv, _check_total_risk] — 掃描交易信號
  - Method: `_check_btc_trend(self)` -> Optional[str] — Fetch BTC 1D EMA20/50 trend. Returns 'LONG', 'SHORT', or None on failure.
  ...
  - Method: `_execute_trade(self, symbol, signal_details, signal_type, tier_multiplier, df_signal)` [Calls: _extract_fill_price, _check_btc_trend, _build_log_base, _validate_position_size, _get_close_side, _place_hard_stop_loss, _save_positions, _futures_create_order] — 執行開倉
  - Method: `monitor_positions(self)` [Calls: _handle_v53_reduce, _update_hard_stop_loss, _handle_close, _build_log_base, _handle_stage3, _handle_stage2, _save_positions, fetch_ohlcv, fetch_ticker] — 監控持倉
  - Method: `_fetch_exchange_stop_map(self)` -> Dict[str, float] — 從交易所取得開放中的止損單。
  - Method: `_adopt_ghost_positions(self)` [Calls: _save_positions, _fetch_exchange_stop_map] — 啟動後一次性接管幽靈倉位（exchange 有、positions.json 未記錄）。
  - Method: `_sync_exchange_positions(self)` [Calls: _save_positions] — 交易所倉位 reconciliation（每次 monitor_positions 都執行）。
  - Method: `_handle_close(self, pm, current_price)` -> bool [Calls: _build_log_base, _save_positions, _futures_close_position, fetch_ticker, _calculate_pnl] — 處理平倉。
  - Method: `_handle_stage2(self, pm, current_price, df_1h)` [Calls: _extract_fill_price, _validate_position_size, _get_close_side, _refresh_stop_loss, _futures_create_order] — 處理 Stage 2 加倉
  - Method: `_handle_stage3(self, pm, current_price, df_1h)` [Calls: _extract_fill_price, _validate_position_size, _get_close_side, _refresh_stop_loss, _futures_create_order] — 處理 Stage 3 加倉
  - Method: `_handle_v53_reduce(self, pm, pct, label, current_price)` [Calls: _extract_fill_price, _build_log_base, _refresh_stop_loss, _futures_close_position, _calculate_pnl] — 處理 V5.3 減倉
  - Method: `startup_diagnostics(self)` -> bool [Calls: fetch_ohlcv] — 啟動診斷
  - Method: `run(self)` [Calls: _sync_exchange_positions, _adopt_ghost_positions, startup_diagnostics, scan_for_signals, _save_positions, monitor_positions] — 主運行循環
...
**Dependencies:** `logging, datetime.(datetime, timezone), typing.(TYPE_CHECKING, Dict, List, Optional, Any), dataclasses.(dataclass, field, asdict), trader.strategies.(TradingStrategy)`
...
**Dependencies:** `logging, pandas, typing.(Tuple, Optional, Dict), trader.config.(Config), trader.structure.(StructureAnalysis)`
...
**Dependencies:** `logging, typing.(Optional), trader.config.(Config), trader.infrastructure.api_client.(BinanceFuturesClient), trader.risk.manager.(PrecisionHandler)`
...
**Dependencies:** `ccxt, math, time, logging, decimal.(Decimal, ROUND_DOWN), typing.(Dict, List, Optional, Tuple), trader.config.(Config), trader.infrastructure.api_client.(BinanceFuturesClient), trader.indicators.technical.(DynamicThresholdManager)`
### Class: `PrecisionHandler` — 交易所精度處理類
  ...
  - Method: `__init__(self, exchange)` [Calls: _load_exchange_info, load_markets]
...
**Dependencies:** `__future__.(annotations), logging, abc.(ABC, abstractmethod), enum.(Enum), typing.(TYPE_CHECKING, Dict, Optional, Type, TypedDict), pandas, trader.positions.(PositionManager)`
...
**Dependencies:** `__future__.(annotations), logging, datetime.(datetime, timezone), typing.(TYPE_CHECKING), pandas, trader.positions.(PositionManager), trader.strategies.base.(Action, TradingStrategy, DecisionDict, _apply_common_pre), trader.strategies.base.(StrategyFactory)`
...
**Dependencies:** `__future__.(annotations), logging, datetime.(datetime, timezone), typing.(TYPE_CHECKING, Optional), pandas, trader.positions.(PositionManager), trader.strategies.base.(Action, TradingStrategy, DecisionDict, _apply_common_pre), trader.strategies.base.(StrategyFactory)`
...
**Dependencies:** `logging, pandas, numpy, typing.(Dict, Optional, Tuple), pandas_ta, trader.config.(Config)`
...
**Dependencies:** `time, logging, requests, trader.config.(Config)`
...
**Dependencies:** `html, logging, requests, typing.(Dict), trader.config.(Config)`
...
**Dependencies:** `html, logging, time, datetime.(datetime, timezone), typing.(Optional), requests, trader.config.(Config)`
