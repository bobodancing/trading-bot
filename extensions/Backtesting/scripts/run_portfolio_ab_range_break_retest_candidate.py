"""Run A+B promoted baseline plus the Phase 4D range-break retest candidate."""
...
VARIANT = "slot_a_b_promoted_plus_range_break_retest"
...
DEFAULT_RESULTS_ROOT = BACKTEST_ROOT / "results" / "portfolio_ab_range_break_retest_candidate"
DEFAULT_SUMMARY = DEFAULT_RESULTS_ROOT / "portfolio_ab_range_break_retest_candidate_summary.json"
DEFAULT_REPORT = REPO_ROOT / "reports" / "weekly_profit_phase4d_range_break_retest_candidate_backtest.md"
...
def _candidate_read(candidate: dict[str, Any]) -> str:
    if int(candidate["trades"]) == 0:
        return "The candidate emitted no closed trades in the combined run."
    if float(candidate["net_pnl"]) >= 0.0:
        return "The candidate preserved positive standalone economics in the combined run."
    return "The candidate added realized trades but damaged standalone economics in this run."
...
def run_range_break_retest_candidate(
    *,
    ...
    if rerun or not (cell_dir / "summary.json").exists():
        print(
            "[PortfolioABRangeBreakRetestCandidate] "
    ...
    payload = {
        "schema": "strategy_plugin_portfolio_ab_range_break_retest_candidate.v1",
    ...
    print(f"[PortfolioABRangeBreakRetestCandidate] summary={summary_path}")
    print(f"[PortfolioABRangeBreakRetestCandidate] report={report_path}")
    return summary_path, report_path


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run A+B promoted baseline plus range-break retest candidate."
    )
    ...
    run_range_break_retest_candidate(
        start=args.start,
        end=args.end,
