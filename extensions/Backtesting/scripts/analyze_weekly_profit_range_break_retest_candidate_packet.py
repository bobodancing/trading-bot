"""Build weekly packet evaluation for A+B plus the Phase 4D range-break candidate."""
...
DEFAULT_JSON = (
    ...
    / "weekly_profit_range_break_retest_candidate_packets.json"
)
DEFAULT_REPORT = (
    REPO_ROOT / "reports" / "weekly_profit_phase4d_range_break_retest_candidate_packet.md"
)
...
SLOT_LABELS = {
    ...
    CANDIDATE: "Range-Break Retest Candidate",
}
...
def build_range_break_retest_candidate_packet_payload(
    *,
    ...
    payload = {
        "schema": "strategy_plugin_weekly_profit_range_break_retest_candidate_packets.v1",
...
def _strict_read(comparison: dict[str, Any]) -> str:
    comparable = bool(comparison.get("contract_grade_comparable"))
    if not comparable:
        return "The candidate packet was generated, but no comparable Phase 3 baseline packet was available."
    ...
    if active_delta > 0.0 and net_delta >= 0.0:
        return (
            "The candidate improves cadence without damaging the latest contract-grade "
            "rolling net after-fee PnL versus Phase 3 baseline."
        )
    if active_delta > 0.0:
        return (
            "The candidate improves cadence but damages economics. It should stay "
            "research-only until trade-level attribution proves this is repairable."
        )
    return (
        "The candidate does not improve latest contract-grade cadence versus Phase 3 "
...
def _render_report(payload: dict[str, Any]) -> str:
    ...
    comparable = bool(comparison.get("contract_grade_comparable"))
    packet_json = (payload.get("artifact") or {}).get("packet_json", str(DEFAULT_JSON))
    lines = [
        "# Weekly Profit Phase 4D Range-Break Retest Candidate Packet",
        "",
        f"Date: {datetime.now(timezone.utc).date().isoformat()}",
        "Branch: `codex/post-promotion-control-20260430`",
        f"Variant: `{payload['source']['variant']}`",
        ...
        (
            ...
            "A+B+range-break-retest combined run."
        ),
        "",
        _strict_read(comparison),
        "",
    ...
    lines.extend(
        [
            ...
            "## Artifact",
            "",
            f"- Candidate packet JSON: `{packet_json}`",
...
def write_range_break_retest_candidate_packet(
    *,
    source_path: Path = DEFAULT_SUMMARY,
    baseline_packet_path: Path = BASELINE_PACKET_JSON,
    ...
    payload = build_range_break_retest_candidate_packet_payload(
    ...
    payload["artifact"] = {"packet_json": str(json_path), "report": str(report_path)}
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build weekly packet for A+B plus range-break retest candidate."
    )
    parser.add_argument("--source", type=Path, default=DEFAULT_SUMMARY)
    parser.add_argument("--baseline-packet", type=Path, default=BASELINE_PACKET_JSON)
    ...
    payload = write_range_break_retest_candidate_packet(
        ...
        json.dumps(
            {
                ...
                "latest_contract_grade_state": (
                    payload["latest_contract_grade_packet"]["decision"]["state"]
                    if payload["latest_contract_grade_packet"]
                    else None
                ),
